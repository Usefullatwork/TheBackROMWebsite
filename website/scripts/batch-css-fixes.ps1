# Batch CSS Fixes for Blog Design Standardization
# Applies: Mobile CTA orange, stat-grid sizes, infographic width, location-section CSS

$websiteRoot = $PSScriptRoot | Split-Path -Parent

# Get all HTML files (excluding backups)
$blogFiles = Get-ChildItem (Join-Path $websiteRoot "blogg\*.html") -File -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne "index.html" }
$hofteFiles = Get-ChildItem (Join-Path $websiteRoot "plager\hofte\*.html") -File -ErrorAction SilentlyContinue
$svimmelhetFiles = Get-ChildItem (Join-Path $websiteRoot "plager\svimmelhet\*.html") -File -ErrorAction SilentlyContinue

$allFiles = @()
if ($blogFiles) { $allFiles += $blogFiles }
if ($hofteFiles) { $allFiles += $hofteFiles }
if ($svimmelhetFiles) { $allFiles += $svimmelhetFiles }
...
foreach ($file in $allFiles) {
    ...
    try {
        ...
        # Fix 1: Mobile CTA background - Change teal to orange
        ...
        if ($content -match 'mobile-sticky-cta a.*background:\s*white.*color:\s*var\(--hub-primary\)') {
        ...
        if ($content -match 'stat-number.*font-size:\s*2rem') {
            $content = $content -replace '(\.stat-number\s*\{[^}]*font-size:\s*)2rem', '${1}1.5rem'
            $modified = $true
            Write-Host "  - Fixed stat-number font size (2rem)" -ForegroundColor Green
        }

        # Fix 4: Stat-number font sizes (1.8rem -> 1.5rem)
        if ($content -match 'stat-number.*font-size:\s*1\.8rem') {
            $content = $content -replace '(\.stat-number\s*\{[^}]*font-size:\s*)1\.8rem', '${1}1.5rem'
            $modified = $true
            Write-Host "  - Fixed stat-number font size (1.8rem)" -ForegroundColor Green
        }

        # Fix 5: Stat-number font sizes (28px -> 1.5rem)
        if ($content -match 'stat-number.*font-size:\s*28px') {
            $content = $content -replace '(\.stat-number\s*\{[^}]*font-size:\s*)28px', '${1}1.5rem'
            $modified = $true
            Write-Host "  - Fixed stat-number font size (28px)" -ForegroundColor Green
        }

        # Fix 6: Stat-label font sizes (0.9rem -> 0.75rem)
        if ($content -match 'stat-label.*font-size:\s*0\.9rem') {
            $content = $content -replace '(\.stat-label\s*\{[^}]*font-size:\s*)0\.9rem', '${1}0.75rem'
            $modified = $true
            Write-Host "  - Fixed stat-label font size (0.9rem)" -ForegroundColor Green
        }

        # Fix 7: Stat-label font sizes (0.85rem -> 0.75rem)
        if ($content -match 'stat-label.*font-size:\s*0\.85rem') {
            $content = $content -replace '(\.stat-label\s*\{[^}]*font-size:\s*)0\.85rem', '${1}0.75rem'
            $modified = $true
            Write-Host "  - Fixed stat-label font size (0.85rem)" -ForegroundColor Green
        }

        # Fix 8: Stat-label font sizes (12px -> 0.75rem)
        if ($content -match 'stat-label.*font-size:\s*12px') {
            $content = $content -replace '(\.stat-label\s*\{[^}]*font-size:\s*)12px', '${1}0.75rem'
            $modified = $true
            Write-Host "  - Fixed stat-label font size (12px)" -ForegroundColor Green
        }

        # Fix 9: Infographic max-width (800px -> 100%)
        ...
        if (($content -match '<style>') -and ($content -notmatch '\.location-header')) {
